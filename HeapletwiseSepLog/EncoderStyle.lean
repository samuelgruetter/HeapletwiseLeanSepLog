...
structure Heaplet {Val : Type}
  (encode : Val -> Address -> Option Mem) (val: Val) (addr : Address) : Type
where
  mem : Mem
  connection : encode val addr = some mem
...
variable (myRecord : MyRecord -> Address -> Option Mem)

variable (uint : Nat -> Nat -> Address -> Option Mem)

variable (array : forall {E : Type}, (E -> Address -> Option Mem) -> Nat ->
  List Nat -> Address -> Option Mem)
